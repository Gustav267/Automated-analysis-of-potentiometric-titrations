...
def create_plot(vol_naoh1: list[float], ph_wert1: list[float], ax1: Axes):
    ...
    def zwe_abl(x, A, D, C, B, G, F, H):
        return -2 * B**2 * G**2 * (x / C) ** (B * G) * (x / C) ** (B * G - 1) * (
            A - D
    ...
    def finde_x_bei_steigung_eins(bereich):
        res = root_scalar(
            ...
            bracket=bereich,
            fprime=lambda x: (
                zwe_abl(x, **result.best_values) / (vol_max - vol_min) ** 2
            ),
    ...
    x_scaled_nullstellen_zweiter_ableitung = root_scalar(
        lambda x: zwe_abl(x, **result.best_values), x0=C_start, method="newton"
    ).root
    ...
    ax1.scatter(
        vol_naoh, ph_wert, color="blue", label="Messpunkte", marker="x", zorder=1, s=20
    )
    ax1.plot(
        x_linespace_ml,
        ph_zu_linespace,
    ...
    ax1.scatter(
        Aep_gesch,
        seven_pl(C_start, **result.best_values),
        color="gray",
        label=f"Parameter C bei {Aep_gesch:.3f} ml",
        marker="_",
        zorder=1,
        s=20,
    )
    ...
    ax1.scatter(
        ...
        color="purple",
        label=f"2. Ableitung 0 bei {vol_nullstellen_zweiter_ableitung:.3f} ml",
        marker="_",
        zorder=1,
        s=20,
    )
    if len(x_steigung_eins) > 1:
        ax1.scatter(
            ...
            aequivalenzpunkt[1],
            color="red",
            label=f"Äq. t. bei {aequivalenzpunkt[0]:.3f} ml",
            marker="_",
            zorder=3,
            s=20,
        )
